...
class _NumberConverterHomePageState extends State<NumberConverterHomePage> {
  ...
  void _convertNumber() {
    ...
    try {
      if (_selectedFromBase == 'Decimal' && input.contains('.')) {
        // Handle floating point decimal input
        if (!RegExp(r'^[0-9]*\.?[0-9]+$').hasMatch(input)) {
          throw FormatException('Invalid decimal number');
        }
        double decimalValue = double.parse(input);
        setState(() {
          _binaryResult = _convertFractional(decimalValue, 2);
          _octalResult = _convertFractional(decimalValue, 8);
          _decimalResult = decimalValue.toString();
          _hexResult = _convertFractional(decimalValue, 16).toUpperCase();
          _errorMessage = '';
        });
      } else {
      ...
      setState(() {
        ...
        } else if (_selectedFromBase != 'Decimal' && input.contains('.')) {
          _errorMessage = 'Only integer values allowed for $_selectedFromBase';
        } else {
  ...
  /// Converts a floating-point decimal to the given base (2, 8, 16).
  /// Returns a string representation with up to [precision] fractional digits.
  String _convertFractional(double value, int base, {int precision = 8}) {
    int integerPart = value.truncate();
    double fractionalPart = value - integerPart;

    String intStr = integerPart.toRadixString(base);

    if (fractionalPart == 0) return intStr;

    String fracStr = '';
    int count = 0;
    while (fractionalPart != 0 && count < precision) {
      fractionalPart *= base;
      int digit = fractionalPart.truncate();
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text(
                        ...
                        'Note: Decimal input supports decimal points. Other systems require integer values.',
